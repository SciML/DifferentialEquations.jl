...
for k in eachindex(tols)
  ...
  @progress for j = 1:M
    println("j = $j")
    Wtmp = pmap((i)->(solve(prob::SDEProblem,[0,T],Δt=1/2^(4),alg="SRIW1Optimized",adaptive=true,abstol=tol,reltol=0,adaptiveAlg="RSwM1",qmax=qmax).W),1:N)
...
for k in eachindex(tols)
  ...
  @progress for j = 1:M
    println("j = $j")
    Wtmp = pmap((i)->solve(prob::SDEProblem,[0,T],Δt=1/2^(4),alg="SRIW1Optimized",adaptive=true,abstol=tol,reltol=0,adaptiveAlg="RSwM2",qmax=qmax).W,1:N)
...
for k in eachindex(tols)
  ...
  @progress for j = 1:M
    println("j = $j")
    Wtmp = pmap((i)->solve(prob::SDEProblem,[0,T],Δt=1/2^(4),alg="SRIW1Optimized",adaptive=true,abstol=tol,reltol=0,adaptiveAlg="RSwM3",qmax=qmax).W,1:N)
    Wends = Float64[Wtmp[i] for i in 1:N]
    kssol = ApproximateOneSampleKSTest(Wends/sqrt(T), Normal())
...
p[1] = Plots.plot(vec(repmat(tols,1,M)'),vec(ps[..,1]),xscale=:log,yscale=:log,
      ...
      guidefont=font(guideSize),titlefont=font(titleSize),tickfont=font(tickSize),ylim=(1e-3,1),xlim=(1e-6,1),leg=false)
